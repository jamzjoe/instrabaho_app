...
class MessagesConversationScreen extends StatefulWidget {
  ...
  @override
  _MessagesConversationScreenState createState() => _MessagesConversationScreenState();
}
...
class _MessagesConversationScreenState extends State<MessagesConversationScreen> {
  ...
  final List<Message> messages = [
    Message(
      ...
      datetime: DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 13, 6)
    ),
    Message(
      sender: 'User',
      chat: 'I\'m good, thanks! How about you?',
      datetime: DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 13, 10)
    ),
    Message(
      sender: 'Friend',
      chat: 'I\'m doing well, thanks for asking!',
      datetime: DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 13, 15)
    )
  ];
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isUserMessage ? Color(0xff3e1693) : Color(0xfffceed4),
  ...
  void sendMessage(String chat){
    ...
    if(chat.isNotEmpty){
      ...
      setState((){
        messages.add(Message(sender: "User", chat: chat, datetime: DateTime.now()));
      });
